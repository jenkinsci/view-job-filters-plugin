...
public class JobType<T extends Job> {
    ...
    public static final JobType<Job> SCM_TRIGGER_ITEM = new JobType("hudson.model.Job", "jenkins.triggers.SCMTriggerItem");
    ...
    private JobType(String className, String... interfaceNames) {
        try {
            ...
            interfaces = new Class[interfaceNames.length + 1];
            for (int i = 0; i < interfaceNames.length; i++) {
                interfaces[i] = (Class<T>) Class.forName(interfaceNames[i]);
            }
            interfaces[interfaces.length - 1] = (Class)TopLevelItem.class;
            available = true;
        } catch (Throwable t) {
            jobClass = null;
            interfaces = null;
